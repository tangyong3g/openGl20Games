...
{	
    ...
    public void initShaderForShadow(MySurfaceView mv)
    {
        ...
        //获取程序中顶点位置属性引用  
        maPositionHandleForShadow = GLES20.glGetAttribLocation(mProgramForShadow, "aPosition");
        //获取程序中总变换矩阵引用
        muMVPMatrixHandleForShadow = GLES20.glGetUniformLocation(mProgramForShadow, "uMVPMatrix");  
        //获取位置、旋转变换矩阵引用
        muMMatrixHandleForShadow = GLES20.glGetUniformLocation(mProgramForShadow, "uMMatrix"); 
        //获取程序中光源位置引用
        maLightLocationHandleForShadow=GLES20.glGetUniformLocation(mProgramForShadow, "uLightLocation");
    }
    ...
    {        
   	 	//制定使用某套着色器程序
   	 	GLES20.glUseProgram(mProgramForShadow);
        //将最终变换矩阵传入着色器程序
        GLES20.glUniformMatrix4fv(muMVPMatrixHandleForShadow, 1, false, MatrixState.getFinalMatrix(), 0);      
        //将位置、旋转变换矩阵传入着色器程序
        GLES20.glUniformMatrix4fv(muMMatrixHandleForShadow, 1, false, MatrixState.getMMatrix(), 0);   
        //将光源位置传入着色器程序   
        GLES20.glUniform3fv(maLightLocationHandleForShadow, 1, MatrixState.lightPositionFB);
        ...
        //启用顶点位置、法向量数据
        GLES20.glEnableVertexAttribArray(maPositionHandleForShadow);  
        //绘制加载的物体
        GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 0, vCount); 
    }
}
