...
public class GLGameView extends GLSurfaceView 
{
    ...
    public boolean onTouchEvent(MotionEvent e) 
	{
		if(isGameOn)
		{
			...
			case MotionEvent.ACTION_DOWN:
	            ...
	            {
					if(x>280*ratio_width&&x<540*ratio_width&&y>186*ratio_height&&y<270*ratio_height)//下一任务
					{
						...
						{
							Toast.makeText(activity, "没有新的行动,请点击菜单按钮,返回菜单界面!!!", Toast.LENGTH_LONG).show();
						}
 					}
					else if(x>280*ratio_width&&x<540*ratio_width&&y>270*ratio_height&&y<370*ratio_height)//菜单
					{
						Toast.makeText(activity, "--------菜单--------------------", 1000).show();
						kThread.flag_go=false;//线程标志位
	            ...
	            {
	            	if(x>280*ratio_width&&x<540*ratio_width&&y>175*ratio_height&&y<270*ratio_height)//重新挑战
					{
						kThread.flag_go=false;//线程标志位
						plane.blood=plane_blood;
						goTime=actionTimeSpan[mapId-3];
	            ...
	            else//--------------------然后是其他情况
	            {
					...
					{
						if(x>200*ratio_width&&x<600*ratio_width&&y>174*ratio_height&&y<200*ratio_height)//------继续按钮
						{
							...
							else if(isVideo&&!isVideoPlaying&&(!isVideoPlaying&&isTrueButtonAction))//如果在视频播放界面
							{
								isVideoPlaying=true;
								if(isMusicOn==0&&!activity.bgMusic[1].isPlaying())
								{
						...
						else if(x>200*ratio_width&&x<600*ratio_width&&y>250*ratio_height&&y<310*ratio_height)//返回菜单菜单
						{
							kThread.flag_go=false;//线程标志位
							missile_ZOffset=missile_ZOffset_Ori;
							missile_rotation=0;
					...
					{
						if(x>200*ratio_width&&x<600*ratio_width&&y>124*ratio_height&&y<180*ratio_height)//下一关按钮
						{
							kThread.flag_go=false;//线程标志位
							plane.blood=plane_blood;
							if(mapId+1<3)
							...
							{
								Toast.makeText(activity,"没有新的战役,请点击菜单按钮,返回菜单界面!!!", Toast.LENGTH_LONG).show(); 
							}
						}
						else if(x>200*ratio_width&&x<600*ratio_width&&y>200*ratio_height&&y<260*ratio_height)//重玩按钮
						{
							kThread.flag_go=false;//线程标志位
							plane.blood=plane_blood;
							initMap();//重新创建高射炮和坦克
						...
						else if(x>200*ratio_width&&x<600*ratio_width&&y>300*ratio_height&&y<360*ratio_height)//返回菜单按钮
						{
							kThread.flag_go=false;//线程标志位
							missile_ZOffset=missile_ZOffset_Ori;
							missile_rotation=0;
				...
				if(isVideo&&!(!isVideoPlaying&&isTrueButtonAction))//如果是视频播放界面
				{
	        		...
	        		if(!is_button_return&&x>680*ratio_width&&y>404*ratio_height)//----进入游戏
	        		{
	        	...
	        	{
	        		WEAPON_INDEX=(WEAPON_INDEX+1)%tex_button_weaponId.length;//更换武器图片
	        	} 
	        	//向上按钮
					...
					if(doorState==2)//如果当前为关舱状态
					{
						if(curr_menu_index==1)//如果当前为设置界面
						{
							...
							if(x>SETTING_BUTTON_AREA1[0]&&x<SETTING_BUTTON_AREA1[1]&&y>SETTING_BUTTON_AREA1[2]&&y<SETTING_BUTTON_AREA1[3])
							{
								...
								if(isMusicOn==1&&activity.bgMusic[0].isPlaying())//如果菜单界面的音乐正在播放
								{
									activity.bgMusic[0].pause();
								}
    ...
    {   
        ...
        {	
            ...
            ShaderManager.loadFirstViewCodeFromFile(GLGameView.this.getResources());//加载shader
        ...
        public void drawAll()
        {
    		...
    		{//如果原Z轴负方向飞行
    			...
    			else if(curr_PlaneZ>MapArray[mapId].length*WIDTH_LALNDFORM)//如果是在进入岛的过程中
    			{
		...
		public void drawGameDialog()
		{
            ...
            if((isGameMode==0&&isCrash&&!isVideo)||(is_button_return&&!isVideo)||(isVideo&&!isVideoPlaying&&isTrueButtonAction))//如果是飞机坠毁，或者按下了返回按钮
            {
	    ...
	    public void drawGameMenu()
	    {
	        ...
	        if(1==isMenuLevel)//如果当前不是位于地图选择界面
	        {
		        ...
		        if(doorState==2&&curr_menu_index==2)
		        {
		        	...
		        	{
		        		float curr_y=0.22f-RANK_NUMBER_HEIGHT*1.3f*i+rank_move;//当前y位置
...
//		        backgroundRect.drawSelf(backgroundId_02);
	...
	public void init_All_Texture(int index)
	{
		...
		case 1:
			...
			tex_plane_select_head=initTexture(getResources(), R.drawable.plane_select_head,false);//标题纹理
			break;
		case 2:
        	...
        	tex_menu_two_okId[0]=initTexture(getResources(), R.drawable.plane_select_ok,false);//菜单二确定按钮
        	tex_menu_two_okId[1]=initTexture(getResources(), R.drawable.plane_select_ok_down,false);//菜单二确定按钮
			break;
		case 3:
        	tex_menu_two_leftId[0]=initTexture(getResources(), R.drawable.menu_two_left_up,false);//左按按钮
        	tex_menu_two_leftId[1]=initTexture(getResources(), R.drawable.menu_two_left_down,false);//左按按钮
        	tex_menu_two_rightId[0]=initTexture(getResources(), R.drawable.menu_two_right_up,false);//右按按钮
        	tex_menu_two_rightId[1]=initTexture(getResources(), R.drawable.menu_two_right_down,false);//右按按钮
        	tex_menu_two_plane_iconId[0][0]=initTexture(getResources(), R.drawable.menu_two_planeicon_one_up,false);//第一幅飞机图片
            tex_menu_two_plane_iconId[0][1]=initTexture(getResources(), R.drawable.menu_two_planeicon_one_down,false);//第二幅飞机图片
            tex_menu_two_plane_iconId[1][0]=initTexture(getResources(), R.drawable.menu_two_planeicon_two_up,false);//第三幅飞机图片
            tex_menu_two_plane_iconId[1][1]=initTexture(getResources(), R.drawable.menu_two_planeicon_two_down,false);//第一幅飞机图片
            tex_menu_two_plane_iconId[2][0]=initTexture(getResources(), R.drawable.menu_two_planeicon_three_up,false);//第二幅飞机图片
            tex_menu_two_plane_iconId[2][1]=initTexture(getResources(), R.drawable.menu_two_planeicon_three_down,false);//第三幅飞机图片
			break;  
		case 4:
			//灯塔纹理
		    ...
		    tex_nightId=initTexture(getResources(), R.drawable.skynight,false);//夜空
			//海水纹理
		...
		case 5:
			...
			break;
		case 6:
			tex_special_action_bgId=initTexture(getResources(), R.drawable.map_selected_bg_action,false);//特别行动背景图
			treeTexId_2=initTexture(getResources(), R.drawable.tree2,false);;//树纹理
			treeTexId=initTexture(getResources(), R.drawable.tree,false);;//树纹理
			//锁定矩形纹理
		...
		case 7: 
			//界面菜单各种纹理，飞机坠毁后的
		    tex_menu_text=initTexture( getResources(),R.drawable.caidanfanhuianniu,false);//文字
		    tex_menu_text_win=initTexture( getResources(),R.drawable.caidanshengli,false);//赢了时的菜单文字
		    //视频播放按钮，演示过程按钮
		...
		case 8:
			...
			//武器图标
			tex_button_weaponId[0]=initTexture(getResources(), R.drawable.bullet_button,false);//子弹按钮图标
			tex_button_weaponId[1]=initTexture(getResources(), R.drawable.missile_button,false);//导弹按钮图标
			//向上按钮纹理
			...
			break;
		case 9:
			tex_musicId[1]=initTexture(getResources(), R.drawable.music_on,false);//是否开启音乐纹理
		    tex_musicId[0]=initTexture(getResources(), R.drawable.music_off,false);//是否开启音乐纹理
		    tex_soundId[1]=initTexture(getResources(), R.drawable.sounds_on,false);//是否开启特效声音纹理
		...
		case 11:
			//特殊行动成功失败对话框
		    tex_actionWinId=initTexture(getResources(), R.raw.action_win,false);//特殊行动成功对话框
		    tex_actionFailId=initTexture(getResources(), R.raw.action_fail,false);//失败对话框
		    //-----游戏开始前的说明文字
		...
		case 12:
			...
			break;
		case 13:
			baoZhaXiaoguo=initTexture( getResources(),R.drawable.baozaoxiaoguo,false);//爆炸效果
		    baoZhaXiaoguo2=initTexture( getResources(),R.drawable.baozhazdan,false);//爆炸效果2
		    tex_numberRectId=initTexture( getResources(),R.drawable.number,false);//数字纹理
		    tex_backgroundRectId=initTexture( getResources(),R.drawable.xuebeijing,false);//血背景图片
		    tex_damId=initTexture(getResources(),R.drawable.dam,false);//大坝
			break;
		case 14:
			//标志其位置的在仪表盘上的
			tex_mark_tanke=initTexture( getResources(),R.drawable.marktanke,false);//坦克和高射炮仪表盘图标
			tex_mark_ackId=initTexture( getResources(),R.drawable.markask,false);//敌机仪表盘图标
			tex_mark_arsenalId=initTexture( getResources(),R.drawable.markarsenal,false);//军火库仪表盘图标
			tex_mark_planeId=initTexture( getResources(),R.drawable.markplane,false);//玩家飞机仪表盘图标
			break;
		case 15:
		...
		case 17:
		    ...
		    tex_front_frameId=initTexture(getResources(), R.drawable.front_frame,false);//导弹菜单下的背景图
		    tex_front_coverId=initTexture(getResources(), R.drawable.front_cover,false);//导弹菜单下的背景图
		    tex_menu_doorId=initTexture(getResources(), R.drawable.menu_door,false);//导弹菜单下的机舱门背景
			break;
		case 18:
		  	tex_soundId[0]=initTexture(getResources(), R.drawable.sounds_off,false);//是否开启特效声音纹理
		    tex_vibrateId[1]=initTexture(getResources(), R.drawable.vibrate_on,false);//是否开启震动纹理
		    tex_vibrateId[0]=initTexture(getResources(), R.drawable.vibrate_off,false);//是否开启震动纹理
		    tex_helpId=initTexture(getResources(), R.drawable.helpview,false);//帮助界面Id
		    break;
		case 19:
			tex_aboutId=initTexture(getResources(), R.drawable.aboutview,false);//关于界面Id
		    tex_mapSelectedBgId=initTexture(getResources(), R.drawable.map_selected_bg,false);//地图选择界面的背景
		    tex_mapId[0]=initTexture(getResources(), R.drawable.yeyingxingdong,false);//地图选择界面的背景
			break;
		case 20:
			tex_mapId[1]=initTexture(getResources(), R.drawable.zhongjifuchou,false);//地图选择界面的背景
		    tex_mapId[2]=initTexture(getResources(), R.drawable.wangzheguilai,false);//地图选择界面的背景
		    tex_rankBgId=initTexture(getResources(), R.drawable.rank_bg,false);//排行榜背景图
		    tex_rankNumberId=initTexture(getResources(), R.drawable.rank_number,false);//排行榜界面的数字
			break;
		} 
	}
	//创建所有的对象
	public void init_All_Object(int index)//第一次进来必须创建的，如果是第二次进来就不需要了
	{
		...
		case 21:
			//------------------------------创建导弹菜单
			missile_menu=new MissileMenuForDraw(ShaderManager.getOnlyTextureShaderProgram());//导弹菜单
			menu_Background=new TextureRect(150, 100, ShaderManager.getWaterTextureShaderProgram(),true,1);//陆地
			menu_clouds=new TextureRect(200, 150, ShaderManager.getWaterTextureShaderProgram(),true,3f);//云彩
			break;
		case 22:
			menu_setting=new TextureRect(SETTING_BUTTON_WIDTH, SETTING_BUTTON_HEIGHT, ShaderManager.getOnlyTextureShaderProgram());//创建机舱门
			helpView=new TextureRect(HELP_WIDTH, HELP_HEIGHT, ShaderManager.getOnlyTextureShaderProgram());//帮助界面
			aboutView=new TextureRect(ABOUT_WIDTH, ABOUT_HEIGHT, ShaderManager.getOnlyTextureShaderProgram());//关于界面
			rank_number=new NumberForDraw(11,RANK_NUMBER_WIDTH,RANK_NUMBER_HEIGHT, ShaderManager.getOnlyTextureShaderProgram());
			map_name=new TextureRect(RANK_MAP_WIDTH, RANK_MAP_HEIGHT, ShaderManager.getOnlyTextureShaderProgram());//排行榜界面的地图的名称
			break;
		case 23:
			plane_select_head=new TextureRect(PLANE_SELECT_HEAD_WIDTH, PLANE_SELECT_HEAD_HEIGHT, ShaderManager.getOnlyTextureShaderProgram());//选飞机场景中的标题栏
            plane_select_plane=new TextureRect(PLANE_SELECT_PLANE_WIDTH, PLANE_SELECT_PLANE_HEIGHT, ShaderManager.getButtonTextureShaderProgram(),1,0);//选飞机场景中的选飞机按钮
            menu_two_game_model_btn=new TextureRect(MENU_TWO_GAME_MODEL_BUTTON_WIDTH, MENU_TWO_GAME_MODEL_BUTTON_HEIGHT, ShaderManager.getButtonTextureShaderProgram(),1,2);//选飞机场景中的选模式按钮
            menu_two_button=new TextureRect(MENU_TWO_BUTTON_WIDTH, MENU_TWO_BUTTON_HEIGHT, ShaderManager.getButtonTextureShaderProgram(),1,0);//选飞机场景中的选模式按钮
		...
		case 26:
			//加载坦克模型
			tanke_body=MoXingJiaZai.loadFromFileVertexOnly("tank_body.obj", getResources(),ShaderManager.getOnlyTextureShaderProgram());//坦克
			break;
		case 27:
		...
		case 30:
			...
			numberRect=new NumberForDraw(10,NUMBER_WIDTH,NUMBER_HEIGHT,ShaderManager.getOnlyTextureShaderProgram());//创建数字
			backgroundRect_blood=new TextureRect(NUMBER_WIDTH*10,NUMBER_HEIGHT,ShaderManager.getStarryXueShaderProgram(),2,0);
			break;
		case 31:
			house=new House(ShaderManager.getOnlyTextureShaderProgram(),backgroundRect_blood,	numberRect);//创建军火库模型
			housePlane=new CubeForDraw(house_length,house_width,house_height,ShaderManager.getOnlyTextureShaderProgram());//创建平房
		    bombRect=new TextureRect(bomb_width,bomb_height,ShaderManager.getOnlyTextureShaderProgram());//爆炸效果纹理
		    bombRectr=new TextureRect(bomb_width/2,bomb_height/2,ShaderManager.getOnlyTextureShaderProgram());//爆炸效果纹理
			break;
		case 32:
			//飞机杯击中的纹理矩形
		    plane_Hit=new TextureRect(ratio*2,2,ShaderManager.getOnlyTextureShaderProgram());
		    menu_Rect=new TextureRect(0.8f,1.2f,ShaderManager.getOnlyTextureShaderProgram());;//飞机爆炸后的菜单显示矩形
		    menu_video=new TextureRect(0.25f*ratio,0.35f,ShaderManager.getOnlyTextureShaderProgram());//播放界面的各个按钮
			break;
		case 33:
			  //标志位置的矩形
		...
		case 34:
			...
			//加载飞机模型
			planeModel[0]=MoXingJiaZai.loadFromFileVertexOnly("feiji11.obj", getResources(),ShaderManager.getOnlyTextureShaderProgram());//飞机
			planeModel[1]=MoXingJiaZai.loadFromFileVertexOnly("feiji22.obj", getResources(),ShaderManager.getOnlyTextureShaderProgram());//飞机
			planeModel[2]=MoXingJiaZai.loadFromFileVertexOnly("feiji33.obj", getResources(),ShaderManager.getOnlyTextureShaderProgram());//飞机
			break;
		case 35:
			//加载坦克模型
			tanke_gun=MoXingJiaZai.loadFromFileVertexOnly("tank_berral1.obj", getResources(),ShaderManager.getOnlyTextureShaderProgram());//坦克
			break;
		case 36:
			//-------------二级菜单中的物体------------------------------------------------------
            ...
            circle_station=new CircleForDraw(ShaderManager.getOnlyColorShaderProgram(), 
            		5, 70,new float[]{0.3f,0.3f,0.3f},ShaderManager.getOnlyTextureShaderProgram());//创建选船界面的额模型
			break;
		case 37:
			front_frame=new TextureRect(ratio*2, 2, ShaderManager.getOnlyTextureShaderProgram());//云彩
			front_cover_button=new TextureRect(MENU_BUTTON_WIDTH, MENU_BUTTON_HEIGHT, ShaderManager.getOnlyTextureShaderProgram());//导弹菜单下的按钮
			front_door=new TextureRect(MENU_DOOR_WIDTH, MENU_DOOR_HEIGHT, ShaderManager.getOnlyTextureShaderProgram());//创建机舱门
			front_door_bg=new TextureRect(MENU_DOOR_WIDTH, MENU_DOOR_HEIGHT*1.8f, ShaderManager.getOnlyTextureShaderProgram());//创建机舱门
			break;
		case 38:
			//创建炮管
	...
	{ 
		...
		MENU_TWO_PLANE_ICON_ONE_XOffset=-ratio+MENU_TWO_PLANE_ICON_WIDTH/2;//按钮的偏移量
		...
		MENU_TWO_PLANE_ICON_TWO_XOffset=MENU_TWO_PLANE_ICON_ONE_XOffset+MENU_TWO_PLANE_ICON_WIDTH;//按钮的偏移量
		...
		MENU_TWO_PLANE_ICON_THREE_XOffset=MENU_TWO_PLANE_ICON_TWO_XOffset+MENU_TWO_PLANE_ICON_WIDTH;//按钮的偏移量
		...
		//-------------------特别行动按钮-----------
		MENU_TWO_ACTION_BUTTON_XOffset=MENU_TWO_WAR_BUTTON_XOffset+MENU_TWO_GAME_MODEL_BUTTON_WIDTH;//按钮的偏移量
